...
# Maximum retries when a sampled geometry collides with one already generated.
_MAX_DEDUP_RETRIES = 10
...
def _geometry_key(
    ...
    """Compute a deduplication key for a geometry sample.

    Rounds continuous parameters to 8 decimal places so that
    floating-point noise doesn't defeat the check.
    ...
    -------
    tuple
        Hashable key suitable for membership testing.
    """
    cl = round(char_length, 8)
    # Include all scalar shape params; skip list-valued ones (block_aspect,
    # fillet_edge_indices) because they don't uniquely identify geometry
    # at the level that matters for dedup.
    extras = []
    for k in sorted(params.keys()):
        v = params[k]
        if isinstance(v, (int, float)):
            extras.append((k, round(float(v), 8)))
    return (family_name, cl, *extras)


def _rebuild_seen_keys(manifest_path: pathlib.Path) -> set[tuple]:
    """Rebuild the geometry-key set from an existing manifest CSV.

    Used on resume so that the dedup check covers previously generated
    samples.
    ...
    -------
    set[tuple]
        Set of geometry keys already present in the manifest.
    """
    import pandas as pd
    ...
    # Deduplicate at base_sample level
    base_df = df.drop_duplicates(subset="base_sample_id")

    keys: set[tuple] = set()
    for _, row in base_df.iterrows():
        family = row["geometry_family"]
        cl = float(row["characteristic_length"])
        params: dict[str, Any] = {}
        if "radius_ratio" in row and pd.notna(row["radius_ratio"]):
            params["radius_ratio"] = float(row["radius_ratio"])
        if "fillet_radius" in row and pd.notna(row["fillet_radius"]):
            params["fillet_radius"] = float(row["fillet_radius"])
        keys.add(_geometry_key(family, cl, params))
    return keys


def _coerce_numeric(obj: Any) -> Any:
    """Recursively coerce string values that look like numbers to float.

    Fixes PyYAML parsing scientific notation (e.g. '432.0e6') as strings
...
class DatasetGenerator:
    ...
    def __init__(
        ...
        self._coherence_tol_overrides = config["validation"].get(
        ...
        # Geometry deduplication set
        self._seen_geometry_keys: set[tuple] = set()
    ...
    def run(self) -> None:
        ...
        if completed:
            ...
            self._seen_geometry_keys = _rebuild_seen_keys(self._manifest_path)
            logger.info(
                "Rebuilt %d geometry dedup keys from manifest",
                len(self._seen_geometry_keys),
            )
        ...
        while n_generated < self._total_target:
            ...
            try:
                # Select family, scale, and shape — with dedup retry
                for _dedup_attempt in range(_MAX_DEDUP_RETRIES):
                    ...
                    gkey = _geometry_key(
                        family_name,
                        scale_sample.characteristic_length,
                        params,
                    )
                    if gkey not in self._seen_geometry_keys:
                        break
                    logger.debug(
                        "Dedup collision for %s (attempt %d), re-sampling",
                        sample_id, _dedup_attempt + 1,
                    )
                else:
                    # Exhausted retries — accept anyway to avoid stalling
                    n_dedup_skipped += 1
                    logger.warning(
                        "Sample %s: dedup retries exhausted, accepting duplicate",
                        sample_id,
                    )

                self._seen_geometry_keys.add(gkey)
                ...
                for variant in variants:
                    ...
                    coherence = run_coherence_check(
                        ...
                        E=self._material.E,
                        nu=self._material.nu,
                        tolerance=self._coherence_tol_overrides.get(
                            family_name, self._coherence_tol,
                        ),
                    )
